...
exports.searchResultsToObject = searchResultsToObject = function(collection, results, populateList, next){
	...
	_.each(results, function(object, i, allObjects){
		...
		for(var l=0;l<populateList.length;l++){
			query.populate(populateList[l].toString())
		}
		query.exec(function(err, realObjects){
			if ( !realObjects ){ return; }
		...
		collection.count(function(err, totalDocs){
			...
			collection.mapReduce(o, function (err, results) {
				...
				if  ( options.returnObjects ){
					searchResultsToObject(collection, results, options.populate||[], next);
				}else{
					next(null, results);
				}
			})// End map-reduce
