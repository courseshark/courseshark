...
if( !mongoose.connection ){
	if ((process.env.NODE_ENV||'development') === 'development' ){
		mongoose.connect(config.db.uri, function(err){if(err){console.log('mongoose-crawl-dev:',err)}})
	}else{
		mongoose.connectSet(config.db.uri, function(err){if(err){console.log('mongoose-crawl-prod:',err)}})
	}
}
