(function(ex){
	...
	function storeSections(dbTerm, objCourse, dbCourse, dbDep){
		...
		for ( var s in objCourse.sections ){
			...
			structs.Section.findOne(search).exec(function(err, foundSection){
				if ( err ){ console.log(err); }
				if ( !foundSection ){
					dbSection = new structs.Section(upsertData);
					dbSection.save(function(err){
						if ( err ){ console.log('-- save new err --',err); return; }
					...
					foundSection.update(upsertData, function(err){
						if ( err ){ console.log("-- update err --", err); return; }

						dbCourse.addSection(foundSection)
						storeTimeslots(dbTerm, objSection, foundSection)
						secEmitter.emit('decreaseCount')
					})
				}
			});
