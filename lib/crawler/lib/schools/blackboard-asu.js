...
var blackboardAsu = module.exports = everySchool.submodule('blackboard-asu')
  ...
  .saveSelf(function(){
    ...
    fs.writeFile(__dirname + '/../../cache/'+this.abbr+'.cache', JSON.stringify(cacheJSON), 'utf8', function(){
  ...
  .getListing(function(err, term){
    var emitter = new EventEmitter()
      ...
      , parser = function(err, window, html, dep){
          ...
          _.each($sectionRows, function(row){
            var _ref
              ...
              , sectionCategory = $row.find('.tooltipRqDesDescrColumnValue').text().trim().split(/,/).filter(function(c){return c!==''})
            ...
            for(_i=0; _i<timeslotCount; _i++){
              ...
              timeslot.days = daySets.map(function(d){ return dayMap[d.toUpperCase()] })
      ...
      , download = function(){
          ...
          // If cache exists use it
          if( fs.existsSync(dep.abbr+'.cache') ){
            return fs.readFile(dep.abbr+'.cache', 'utf8', function (err, data) {
          ...
          self.dl.download({
              ...
              // Download of section
              require('fs').writeFile(dep.abbr+'.cache', html, 'utf8', function (err) {
                if (err) throw err;
              });
    ...
    var self = this
      ...
      , storeSections = function(dbSchool, dbTerm, objCourse, dbCourse, dbDep){
          ...
          for ( s in objCourse.sections ){
            ...
            upsertData = (new Section(search)).toObject();
            ...
            Section.update(search, upsertData, {upsert: true}, (function(dbTerm, objSection, search){
              return function(err, num){
                if ( err ){return console.log("-- Error --",err,objSection,search)}
