...
var blackboardAsu = module.exports = everySchool.submodule('blackboard-asu')
  ...
  .loadTerms(function(err, term){
    ...
    if( fs.existsSync('Terms.html') ){
      return fs.readFile('Terms.html', function (err, data) { callback(err, {close: function(){}}, data) } )
    }else{
      this.dl.download(options, callback)
    }
  ...
  .getListing(function(err, term){
    var emitter = new EventEmitter()
      ...
      , parser = function(err, window, html, dep){
          ...
          //dep.findOrAddCourseByNumber()
          _.delay(function(){emitter.emit('-')}, 1500)
        }
      , download = function(){
          ...
          // If cache exists use it
          if( fs.existsSync('CSE.html') ){
            return fs.readFile('CSE.html', function (err, data) {
          ...
          self.dl.download({
              ...
              // Download of section
              require('fs').appendFile('CSE.html', html, function (err) {
                if (err) throw err;
              });
              (function(err, window, data, dep){
                return parser(err, window, data, dep)
    ...
    emitter.i = self.departments.length;
