...
exports.everyauthBoot = function(everyauth, app){
	...
	everyauth.twitter
			...
			.findOrCreateUser(function(session, accessToken, accessTokenSecret, twitterUserData){
				...
				user.findOrCreateTwitterData(twitterUserData, promise, session);
				return promise;
			})
			.redirectPath('/');
	...
	everyauth.google
			...
			.findOrCreateUser(function (session, accessToken, extra, googleUser){
				var promise = this.Promise();
				user.findOrCreateGoogleData(googleUser, promise, session);
				return promise;
			})
			.redirectPath('/');
	...
	everyauth.facebook
		...
		.findOrCreateUser( function (session, accessToken, accessTokExtra, fbUserData) {
			var promise = this.Promise();
			user.findOrCreateFacebookData(fbUserData, promise, session);
			return promise;
		})
		.redirectPath('/');
