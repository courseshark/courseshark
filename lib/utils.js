...
exports.wantSchool = function(req, res, next) {
  if ( typeof req.school !== "undefined" && req.school !== null){
    next()
  }

  if ( req.subdomain ){
    School.findOne({ abbr: req.subdomain }).populate('currentTerm').exec(function(err, school){
      if ( err || !school ){
        next()
      }
      req.school = school
    ...
    req.school = req.session.school
    next()
  } else if ( req.user && req.user.school ){
    School.findById(req.user.school).populate('currentTerm').exec(function(err, school){
      if ( err || !school ){
        next()
      }
      req.school = school
...
exports.requireSchool = function(req, res, next) {
  ...
  }else if ( typeof req.school !== "undefined" && req.school !== null){
    next()
  }
...
exports.unserialize = function (data) {
    ...
    that.utf8_decode = function(str_data) {
    ...
    var _unserialize = function (data, offset) {
        ...
        case 'a':
            ...
            break;
        default:
            error('SyntaxError', 'Unknown / Unhandled data type(s): ' + dtype + data);
            break;
        }
