		...
		mandrill.messages_send_template({
			...
			, message:{
				...
				if ( err ){
					...
					console.log('[msg-sent] Sent Email notification',notification.id,'to',{name: notification.user.name, email: notification.email, seats: seats, sname: section.name});
					notification.sent = true;
					notification.lastSent = Date.now();
					notification.save();
				}
				...
				(function(notification, school){
					Term.findOne({_id: notification.section.term}, function(err, term){
						...
						if ( term.active ){
							...
							if ( notification.section.number == 82737 ){
								console.log(school, notification.section);
							}
							crawler[school.abbr].safeUpdateSection(notification.section, 15*1000, (function(notification){
