		...
		$.ajax({url: '/users/search/'+q,
				...
				success: function(res){
					...
					for ( f in res.results ){
						userHtml = window.tmpl($('#template-search-friend-result').html(), {user:res.results[f]});
						$(userHtml).data('user', res.results[f]).appendTo($searchList);
					}
...
function inviteUsers(list, $this){
	...
	$.ajax({url: '/social/friends/invite-to-class/'+$this.data('section'),
			type: 'POST',
